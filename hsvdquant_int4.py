...
def prepare_nunchaku_state(
    ...
    *,
    ...
    _require_shape(
        ...
        "Nunchaku backend does not support activation permutations",
    )
    _require_shape(
        not int(state.get("activation_hadamard_group_size", 0)),
        "Nunchaku backend does not support block Hadamard transforms",
    )
        ...
        with torch.no_grad():
            for name, value in tensors.items():
                ...
                _require_shape(
                    ...
                    f"Nunchaku {name} shape mismatch: {parameter.shape} != {value.shape}",
                )
