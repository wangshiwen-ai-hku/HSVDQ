...
\begin{abstract}
At W4A4, weight and activation errors compound, and post-training methods that
only tune quantization scales and integer codes can leave a substantial,
structured residual error. We therefore retain a small full-precision SVD
bypass to absorb this residual structure and quantize only the remaining
component. Our key finding is that jointly optimizing the
...
second-order code updates improve the grid, yet they can still leave a
substantial accuracy gap and a residual error concentrated in a small number of
directions.

Our starting point is to model this structured residual explicitly without
abandoning an efficient low-bit main path. A small full-precision SVD bypass
captures the coherent component left by scale/code optimization, while the
remaining residual stays in a regular W4A4 layout. SVDQuant demonstrates this principle
...
\begin{itemize}
    \item We identify structured residual error left by scale/code-only W4A4
    PTQ and use a small full-precision SVD bypass to absorb it.
