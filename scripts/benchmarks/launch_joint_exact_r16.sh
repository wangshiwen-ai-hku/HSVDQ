...
launch_one() {
  ...
  local device="$2"
  local log_path="${LOG_DIR}/${calib_dataset}_r16.log"
  ...
  nohup env \
    ...
    bash scripts/benchmarks/run_joint_fw_fa.sh "${calib_dataset}" 16 "${device}" \
    >"${log_path}" 2>&1 &
  echo "$! lambda=0.25 calib=${calib_dataset} rank=16 device=${device} log=${log_path}"
}
